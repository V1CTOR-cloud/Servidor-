...
class DataBase implements IDataBase
{
    ...
    {
        return $this->conn->query("SELECT * FROM " . $table_name)->fetchall(PDO::FETCH_ASSOC);
    }
    
    public function find(string $table_name, string $campo_clave, int $id): array
    {
        return $this->conn->query("SELECT * FROM $table_name WHERE $campo_clave=$id")->fetchall(PDO::FETCH_ASSOC);
    }
